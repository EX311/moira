...
void register_icon(struct icon *icon, int x, int y, char *name, char *action)
{
...
int main(int argc, char *argv[])
{
	...
	for (i=0; i<icon_count; i++) {
		...
		register_icon(&icon[i], x, y, "test", "/root/bmp_viewer");
		x += (2*ICONSIZE);
	}
		...
		for (i=0; i<=icon_count; i++) {
			if ( (t = icon_handle(&icon[i], &samp))) {
				...
				if (pid == 0) {
					...
					printf("Child PID #%d\n", getpid());
#endif
					execl("/root/bmp_viewer", "bmp_viewer", "zero1.bmp", 0);
					printf("This is invinsible!\n");
				} else {
#ifdef DEBUG
					printf("Exec PID #%d\n", pid);
#endif
