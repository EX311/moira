...
//	buttons[2].text = "Split";
	...
	{
		if(isconnected[i])
			serv_sock[i] = tcp_client_connect(serv_addr[i],ip2port(serv_addr[i],8788));
	}
	...
	while(1)
	{
		...
		{
			//Left or Top
			//tell server's that i'm sending my picture
			//which server? where is that? where is my location?

			if(mylocation==2)
			{
				//send to top location 0 server
				write(serv_sock[0],"Receive",8);
				sleep(1);
				write(serv_sock[0],fbuffer,fix.smem_len);
				sleep(1);
			}
			else if(mylocation==3)
			{
				//send to top for demonstration
				write(serv_sock[1],"Receive",8);
				sleep(1);
				write(serv_sock[1],fbuffer,fix.smem_len);
			}
			...
			{
				put_string_center(xres/2,yres/2,"No device detected on my top or left direction!!",1);
			}
		...
		else if(tilt>135)
		{
			...
			{//bottom!!
				write(serv_sock[3],"Receive",8);
				sleep(1);
				write(serv_sock[3],fbuffer,fix.smem_len);
			}
